...
public class CalculateAverage_mellester {
    ...
    private static final int MAPPING_SIZE = 1 << 6;
    private static final List<ByteBuffer> mappings = new ArrayList<>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            for (long offset = 0; offset < size; offset += MAPPING_SIZE) {
                ...
                mappings.add(raf.getChannel().map(FileChannel.MapMode.READ_ONLY, offset, size2));
            }
        ...
        try (ExecutorService threadExecutors = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            for (ByteBuffer mapping : mappings) {
                ...
                threadExecutors.submit(() -> {
                    byte[] bytes = new byte[mapping.remaining()];
                    mapping.get(bytes);
